/**
 * NUST NEXUS — Storj Upload Edge Function
 * Uses native fetch + AWS Signature V4 (no npm packages) for maximum reliability.
 * Secrets: STORJ_ACCESS_KEY, STORJ_SECRET_KEY
 * (set via: supabase secrets set STORJ_ACCESS_KEY=... STORJ_SECRET_KEY=...)
 */
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!file || !path) {
      return new Response(JSON.stringify({ error: "Both 'file' and 'path' fields are required" }), {
        status: 400,
        headers: { ...CORS, "Content-Type": "application/json" },
      });
    }
    ...
    if (!accessKey || !secretKey) {
      console.error("storj-upload: Missing STORJ_ACCESS_KEY or STORJ_SECRET_KEY");
      return new Response(JSON.stringify({ error: "Storage credentials not configured on server" }), {
        status: 500,
        headers: { ...CORS, "Content-Type": "application/json" },
      });
    }

    // ── Upload to Storj via S3 API ─────────────────────────────────────────
    ...
    if (!uploadRes.ok) {
      ...
      console.error("storj-upload: PUT failed", uploadRes.status, errText);
      return new Response(
        JSON.stringify({ error: `Storage PUT failed (${uploadRes.status}): ${errText.slice(0, 200)}` }),
        {
          status: 502,
          headers: { ...CORS, "Content-Type": "application/json" },
        },
    ...
    console.error("storj-upload: unexpected error", e);
    return new Response(
      JSON.stringify({ error: (e as Error).message || "Upload failed" }),
      {
        status: 500,
        headers: { ...CORS, "Content-Type": "application/json" },
      },
